...
import PrayerRequestManagement from './PrayerRequestManagement';
...
export default function AdminDashboard() {
  ...
  const tabs: Record<string, TabProps> = {
    ...
    analytics: {
      ...
      component: (
        <div className="flex flex-col items-center justify-center space-y-4 rounded-lg border border-gray-200 bg-white p-8 text-center dark:border-gray-700 dark:bg-gray-800">
    ...
    prayerRequests: {
      ...
      component: <PrayerRequestManagement />,
    },
    settings: {
      title: 'Settings',
      icon: (
          ...
          <div className="border-t border-gray-200 dark:border-gray-700">
            <nav className="flex flex-col">
              {Object.entries(tabs).map(([key, tab]) => (
                <button
                  key={key}
                  className={`flex items-center space-x-3 border-l-4 px-6 py-3 text-left ${
                    activeTab === key
                ...
                >
                  <div className={`${activeTab === key ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'}`}>{tab.icon}</div>
                  <span>{tab.title}</span>
                </button>
              ))}
            </nav>
